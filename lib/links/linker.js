    ...
    _prepareVirtual() {
        ...
        if (!linker) {
            ...
            Meteor.startup(() => {
                ...
                if (!linker) {
                    throw new Meteor.Error(`You tried setting up an inversed link "${inversedBy}" in "${this.mainCollection._name}" to the "${collection._name}" collection that does not have any links defined`)
                } else {
