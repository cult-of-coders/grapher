...
export default class Linker {
    ...
    _cache() {
        if(this.linkConfig.cache){
            if(this.isVirtual()){
                let inverseLink = this.linkConfig.relatedLinker.linkConfig;
                let type = inverseLink.type == 'many' ? 'many-inverse' : 'inverse';
                this.mainCollection.cache({
                    type:type,
                    collection:this.linkConfig.collection,
                    fields:this.linkConfig.cache,
                    referenceField:inverseLink.field,
                    cacheField:'_' + this.linkName,
                    bypassSchema:true
                });
            } else {
                this.mainCollection.cache({
                    ...
                    collection:this.linkConfig.collection,
                    fields:this.linkConfig.cache,
                    referenceField:this.linkConfig.field,
                    cacheField:'_' + this.linkName,
                    bypassSchema:true
                });
            }
