            ...
            if (removeStorageField) {
                if (collectionNode.isVirtual) {
                    ...
                    _.each(childResults, childResult => {
                        if (isSingle) {
                            dot.pick(collectionNode.linkStorageField, childResult, true);
                            if (nested.length > 0 && _.isEmpty(childResult[root])) {
                                delete childResult[root];
                            }
                        }
                        else {
                            if (nested.length > 0) {
                                const arr = childResult[root] || [];
