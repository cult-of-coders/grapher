...
describe('Query Client Tests', function () {
    ...
    it.only('Should work with queries reactively', function (done) {
        const query = createQuery({
            groups: {
                $filters: {
                    name: {
        ...
        Tracker.autorun(c => {
            if (handle.ready()) {
                c.stop();
                const firstQueryResult = query.fetch();
                const [group1, group2] = firstQueryResult;
                const [author1] = group2.authors;
                const params = { authorId: author1._id, groupId: group1._id };

                Meteor.call('addAuthorToGroupId', params, (error, res) => {
                    const secondQueryResult = query.fetch();
                    const isInGroup = secondQueryResult[0].authors.some(author => author._id === author1._id);
                    assert.isTrue(isInGroup);

                    handle.stop();

                    query.clone().fetch((err, thirdQueryResult) => {
                        const isInGroup = thirdQueryResult[0].authors.some(author => author._id === author1._id);
                        assert.isTrue(isInGroup);
                        done();
                    });
                });
